	...
	public static final Parser<DeferringAction> PARSER = new Parser<>(
			...
			.concat(
					...
					.or(
							...
							.concat(TokenPattern.quoteIgnoreCase("and").concat(AN.omittable()).concat(WORD_LOOP.capture(CAPTURE_SECONDARY)).omittable())
							)
					.or(
							TokenPattern.quoteIgnoreCase("two")
							.concat(NOT_ENDMARKER_LOOP.capture(CAPTURE_PRIMARY, CAPTURE_SECONDARY))))
			.concat(TokenPattern.quoteIgnoreCase("requiring").concat(WORD_LOOP.capture(CAPTURE_REQUIREMENTS)).omittable())
			.concat(ENDMARKER)

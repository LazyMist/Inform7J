...
public class DeferringKind extends DeferringImpl implements IKind {
	...
	public static final Parser<DeferringKind> PARSER = new Parser<>(
			...
			.concat(new TokenPattern.Replacement(DeferringStory.KIND_NAME_REPLACEMENT, false).capture(CAPTURE_SUPERKIND))
			.concat(ENDMARKER),
			DeferringKind::new
			);
	...
	public DeferringKind(DeferringStory story, Source src, TokenString name, Optional<? extends DeferringKind> super_kind) {
		super(story, src);
		NAME = name;
	...
	public String toString() {
		return String.format("%s is a kind of %s", NAME, SUPER_KIND.stream().map(LazyLookup::key).map(TokenString::toString).findAny().orElse(""));
	}
}

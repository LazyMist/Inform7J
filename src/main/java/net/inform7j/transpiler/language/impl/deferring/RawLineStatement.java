...
public record RawLineStatement(TokenString raw, long line, Path src, Source source)
    implements IStatement, Supplier<RawLineStatement> {
    public RawLineStatement(List<SourcedToken> raw) {
        this(
            new TokenString(raw.stream().map(SourcedToken::tok)),
            raw.get(0).line(),
            raw.get(0).src(),
            raw.get(0).source()
        );
